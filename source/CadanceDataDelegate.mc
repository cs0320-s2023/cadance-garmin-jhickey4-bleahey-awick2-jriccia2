...
class CadanceDataDelegate {
  ...
  // Returns the cadence in strides per minute.
  // This value is calculated using the watch's sensors and generally
  // falls between 160 and 180 for a runner.
  ...
  // This value is a floating point number between 0 and 1.
  function getEnergy() as Toybox.Lang.Double {
    return energy;
  }
  ...
  function updateFields(sensorInfo as Sensor.Info) as Void {
    ...
    if (sensorInfo.heartRate != null) {
      ...
      if (dynamics.getRunningDynamics() != null) {
        ...
        cadence = Math.round(
          sensorInfo.cadence * Constants.BIKE_TO_RUN_CAD_RATIO
        ).toNumber();
        cadenceUpdated = true;
      }
      if (cadenceUpdated) {
        try {
          energy =
            Constants.CADENCE_WEIGHT *
              CadanceAlgorithmDelegate.cadenceToEnergy(cadence, height, male) +
            Constants.HR_WEIGHT *
              CadanceAlgorithmDelegate.hrToEnergy(heartRate);
